...
% once
function [ISO,T]=autopeak2D(pks,files,settings,fn_out)
if settings.mode == -1
    mode='neg';
else
    mode='pos';
...
for i=1:length(files)
    ...
    T(i).path=files(i).folder;  %remove the first 3 char Y:"
...
parfor i=1:length(fn)      
    ...
    for j=1:length(pks)
       ...
       try
          ...
          Dnum=10; %Dnum default =10 if no formula provided 
       end       
       tp=get_eic_isodata0(M,pks(j), settings, '2D', Dnum); 
       tp=rmfield(tp,'eic'); 
       iso{j}=tp;
    end 
    ISO(:,i)=iso;
